    ...
    def update(self):
        if self.direction == "up":
            ...
            if self.rect.bottom < road_horizontal.top:
                self.rect.top = road_horizontal.top
        elif self.direction == "down":
            self.rect.y += CAR_SPEED
            if self.rect.top > road_horizontal.bottom - CAR_HEIGHT:
                self.rect.bottom = road_horizontal.bottom - CAR_HEIGHT
        elif self.direction == "left":
            self.rect.x -= CAR_SPEED
            if self.rect.right < road_vertical.left:
                self.rect.left = road_vertical.left
        elif self.direction == "right":
            self.rect.x += CAR_SPEED
            if self.rect.left > road_vertical.right - CAR_WIDTH:
                self.rect.right = road_vertical.right - CAR_WIDTH
